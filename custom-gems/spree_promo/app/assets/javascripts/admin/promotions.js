...
$(document).ready(function() {
  ...
  $('#promotion_event_name').change(function() {
        var a = $('#promotion_event_name').val();
        if (a == "spree.checkout.monthly_coupons") {
$('#promotion_code_field').toggle($('#promotion_event_name').val() == 'spree.checkout.monthly_coupons');
            var fullDate = new Date()
            var twoDigitMonth = ((fullDate.getMonth().length+1) === 1)? (fullDate.getMonth()+1) : '0' + (fullDate.getMonth()+1);
var twoDigitMonth1 = (((fullDate.getMonth()+2).length+1) === 1)? (fullDate.getMonth()+2) : '0' + (fullDate.getMonth()+2);

            var currentDate = fullDate.getFullYear() + "/" + twoDigitMonth + "/" + fullDate.getDate();
             var expires_at = fullDate.getFullYear() + "/" + twoDigitMonth1 + "/" + fullDate.getDate();

            $('#promotion_starts_at').val(currentDate);
            $('#promotion_expires_at').val(expires_at);
        }
        else {
            $('#promotion_starts_at').val('');
            $('#promotion_expires_at').val('');
        }
    });
