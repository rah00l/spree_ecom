...
module Spree
  class Product < ActiveRecord::Base
    ...
    has_many :user_groups, :through => :product_user_groups
    accepts_nested_attributes_for :product_user_groups
    ...
    delegate_belongs_to :master, :sku, :price, :weight, :height, :width, :depth, :is_master#,:feature_product,:see_price_in_the_cart,:retail_price
    delegate_belongs_to :master, :cost_price if Variant.table_exists? && Variant.column_names.include?('cost_price')

    after_create :set_master_variant_defaults
    after_create :set_default_category
    after_save :set_default_category
    after_save :notify_when_product_is_available
    ...
    scope :available_on ,lambda { where("available_on > ?",Time.zone.now)}

    scope :below_minimum_quantity ,lambda { where("count_on_hand < min_quantity")}
...
#        raise master.changed?.inspect
      ...
      def set_default_category
        if self.available_on.blank? || self.available_on <= Time.zone.now
            self.categories << Spree::Category.default_active_categories.find_by_name("New Products")  if Spree::Category.default_active_categories.find_by_name("New Products") && !self.categories.collect(&:name).include?("New Products")
        else
            self.categories << Spree::Category.default_active_categories.find_by_name("Upcoming Products")  if Spree::Category.default_active_categories.find_by_name("Upcoming Products") && !self.categories.collect(&:name).include?("Upcoming Products")
        end
      end

      def notify_when_product_is_available
        if self.has_stock?
          self.out_of_stock_notifiers.where(:stock_notification => nil).each do |set_stock_notification|
            set_stock_notification.update_attributes(:stock_notification => "#{self.updated_at}")
            notify_users_that_product_is_available(self,set_stock_notification.user_id)
          end
        end
      end

      def notify_users_that_product_is_available(product,user_id)
        @mail_method = Spree::MailMethod.first
        user = Spree::User.find_by_id(user_id)
        ProductAvailableNotifierMailer.notify_email(@mail_method, product,user).deliver
      end
  end
end
