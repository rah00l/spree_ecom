  ...
  class Variant < ActiveRecord::Base
    ...
    def custom_price
      ...
      if manu_user_group_price_discount
         discount_price = ((self.price.to_f*manu_user_group_price_discount.to_f)/100).round(2)
        self.price = self.price.to_f - discount_price.to_f
      end

      if current_user && current_user.user_group && current_user.user_group.product_user_groups && self.product && current_user.user_group.product_user_groups.find_by_user_group_id_and_product_id(current_user.user_group.id,self.product.id)
